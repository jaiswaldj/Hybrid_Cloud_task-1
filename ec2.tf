  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/jaiswaldj/Hybrid_Cloud_task-1.git /var/www/html/"
  ]
  } 
  }


  resource "aws_s3_bucket" "terratask1bucket" {
  ...
  resource "aws_s3_bucket_object" "terratask1object" {
  depends_on = [aws_s3_bucket.terratask1bucket,]
  bucket = "terra-task1-bucket-deepak"
  key   = "ab.jpg"
...
resource "aws_cloudfront_distribution" "terratask1_cloudfront_distribution" {
  origin {
    domain_name =aws_s3_bucket.terratask1bucket.bucket_regional_domain_name
